(function (global, factory) {
  ...
  const push$1 = (state) => (val) => state.push(val);

  const pop = (state) => () => state.pop();

  const isEmpty = (state) => () => state.length < 1;

  const clear$1 = (state) => () => state.splice(0);

  const current = (state) => () => state[state.length - 1];
